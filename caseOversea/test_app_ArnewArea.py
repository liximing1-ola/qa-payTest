...
class TestPayCreate(unittest.TestCase):
    ...
    def test_01_arNewAreaVipRoomPay(self, des: str = '阿语大区商业房礼物打赏主播分成 55 场景'):
        ...
        conMysql.updateMoneySql(config.oversea_brokerUid)  # 公会主播账户余额清空
        
        # 2. 房间打赏（给主播）
        data = encodeOverseaData(payType='package', uid=config.oversea_brokerUid, rid=config.oversea_room['business_joy_ar'])
    ...
    def test_02_arNewAreaVipRoomGiveBox(self, des: str = '阿语大区商业房箱子打赏主播 55 分成场景'):
        ...
        data = encodeOverseaData(payType='package',
                            giftId=config.giftId['46'], uid=config.oversea_brokerUid,
        ...
        # 4. 检查余额
        assert_equal(conMysql.selectUserInfoSql('sum_money', config.oversea_payUid), 100)
        assert_len(conMysql.selectUserInfoSql('single_money', config.oversea_brokerUid, money_type='money_cash'), 150)
        assert_equal(conMysql.selectUserInfoSql('single_money', config.oversea_brokerUid, money_type='money_cash'),
