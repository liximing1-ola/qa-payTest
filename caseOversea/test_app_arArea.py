...
class TestPayCreate(unittest.TestCase):
    ...
    def test_01_arAreaVipRoomPay(self, des: str = '阿语大区商业房礼物打赏 37 分成场景'):
        ...
        # 2. 房间打赏
        data = encodeOverseaData(payType='package', rid=config.oversea_room['business_joy_ar'])
        ...
        assert_equal(conMysql.selectUserInfoSql('single_money', config.oversea_testUid, money_type='money_cash'), 420)
    ...
    def test_02_arAreaVipRoomGiveBox(self, des: str = '阿语大区商业房箱子打赏 37 分成场景'):
        ...
        # 4. 检查余额
        assert_equal(conMysql.selectUserInfoSql('sum_money', config.oversea_payUid), 100)
        assert_len(conMysql.selectUserInfoSql('single_money', config.oversea_testUid, money_type='money_cash_b'), 210)
    ...
    def test_03_arAreaChatPay(self, des: str = '阿语大区私聊礼物打赏 28 分成场景'):
        ...
        # 4. 检查余额
        assert_equal(conMysql.selectUserInfoSql('sum_money', config.oversea_payUid), 100)
        assert_equal(conMysql.selectUserInfoSql('single_money', config.oversea_testUid, money_type='money_cash'), 480)
