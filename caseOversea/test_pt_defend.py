...
class TestPayCreate(unittest.TestCase):
    ...
    def test_01_defendPayChangMoney(self):
        """
        用例描述：
        开通个人守护，收益分成在师父收益的基础上为 70:30
        脚本步骤：
        1.构造开通者和被守护者数据 （更新xs_user_money）
        2.开通价值52守护
        3.校验【status code】和返回值【body】状态
        4.检查打赏者余额
        5.检查被打赏者余额,预期：5200 * 0.70 = 3640
        6.检查消费记录
        """
        des = '检查开通个人守护时收益分成场景'
        ...
        Assert.assert_equal(Mysql.selectAllMoneySql(config.pt_testUid), 3640)
        Assert.assert_equal(Mysql.selectPayChangeSql(config.pt_payUid), 5200)
