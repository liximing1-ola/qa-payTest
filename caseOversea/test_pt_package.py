...
class TestPayCreate(unittest.TestCase):
    ...
    def test_02_RoomPayLiveMoney(self):
        ...
        Assert.assert_equal(Mysql.selectMoneySql(config.pt_testUid, 'money_cash_b'), 62)
    ...
    def test_05_mentorPayChange(self):
        ...
        Assert.assert_equal(Mysql.selectAllMoneySql(config.pt_testUid), 62)
        Assert.assert_equal(Mysql.selectPayChangeSql(config.pt_payUid), 100)
        Consts.CASE_LIST[des] = 'pass'
    ...
    def test_06_couponNoStatePayChange(self):
        ...
        Mysql.insertXsUserCommodity(config.pt_payUid, gift_cid, 1)
        Mysql.updateMoneySql(config.pt_payUid, 3000)
        Mysql.updateMoneySql(config.pt_testUid)
        cid = Mysql.getUserCommodityIdSql(gift_cid, config.pt_payUid)
    ...
    def test_07_couponStatePayChange(self):
        ...
        2.房间内打赏（券可抵扣500分）
        3.校验【status code】和返回值【body】状态
        4.检查被打赏者余额和账户，预期为：3000 * 0.70 = 2100
        5.检查打赏者余额,预期为：3000 -2500 = 500
        """
        des = '券已激活(state=1)情况下，检查用券打赏流程'
        gift_cid = 54  # 老司机
        Mysql.deleteUserCommoditySql(config.pt_payUid)
        Mysql.insertXsUserCommodity(config.pt_payUid, gift_cid, 1, 1)
        Mysql.updateMoneySql(config.pt_payUid, 3000)
        Mysql.updateMoneySql(config.pt_testUid)
        cid = Mysql.getUserCommodityIdSql(gift_cid, config.pt_payUid)
        payload = {
        ...
        Assert.assert_code(res['code'], 200)
        Assert.assert_body(res['body'], 'success', 1, reason)
        Assert.assert_equal(Mysql.selectMoneySql(config.pt_testUid), 2100)
