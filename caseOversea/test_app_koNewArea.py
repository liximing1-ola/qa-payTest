...
class TestPayCreate(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        """测试前准备：设置用户大区为韩语区"""
        conMysql.updateUserBigArea(tuple(i for i in config.oversea_user.values()), bigarea_id=4)
    ...
    def test_01_koAreaNoBrokerMemberIMPay(self, des: str = '韩语私聊打赏非主播分成 75%'):
        ...
        assert_equal(
            conMysql.selectUserInfoSql('money_cash_personal', config.oversea_testUid, money_type='money_cash_personal'), 450)
    ...
    def test_02_koAreaBrokerMemberIMPay(self, des: str = '韩语私聊打赏主播分成 70%'):
        ...
        conMysql.updateUserextendMoneyClearSql(config.oversea_brokerUid)  # 非主播钱包附加表账户余额清空
        ...
        # 4. 检查余额
        assert_equal(conMysql.selectUserInfoSql('sum_money', config.oversea_payUid), 100)
        assert_equal(conMysql.selectUserInfoSql('single_money', config.oversea_brokerUid, money_type='money_cash_b'), 420)
    ...
    def test_03_koNewAreaFleetRoomPay(self, des: str = '韩语区家族房礼物打赏主播 70% 分成场景'):
        ...
        # 2. 房间打赏（给主播）
        data = encodeOverseaData(payType='package', rid=config.oversea_room['fleet_normal_ar'], uid=config.oversea_brokerUid)
        res = post_request_session(config.oversea_pay_url, data, token_name='app')
