...
class TestPayCreate(unittest.TestCase):
    ...
    def test_01_roomPayCustomRate_50(self, des='商业房打赏自定义分成:50'):
        ...
        assert_equal(mysql.selectUserInfoSql('single_money', self.customUid,
                                                money_type='money_cash'), 100 * config.rate * 0.5)
        assert_equal(mysql.selectUserInfoSql('single_money', self.ceoUid,
    ...
    def test_02_chatPayCustomRate_80(self, des='私聊打赏自定义分成:80'):
        ...
        """
        mysql.updateMoneySql(config.payUid, money=930, money_cash=30, money_cash_b=30, money_b=10)
        mysql.updateUserMoneyClearSql(self.customUid, self.ceoUid)
        mysql.checkUserBroker(self.customUid, bid=self.ceoUid)  # bid=105002314 被打赏者加入工会
        mysql.checkBrokerUserRate(self.customUid, self.ceoUid, rate=80)  # config.bbc_broker_user_rate 设置分成比
        data = encodeData(payType='chat-gift', uid=self.customUid)
        res = post_request_session(config.pay_url, data)
        assert_code(res['code'])
        assert_body(res['body'], 'success', 1, reason(des, res))
        assert_equal(mysql.selectUserInfoSql('sum_money', config.payUid), 0)
        assert_equal(mysql.selectUserInfoSql('sum_money', self.customUid), 636)
        assert_equal(mysql.selectUserInfoSql('single_money', self.customUid, money_type='money_cash'),
                     1000 * (config.rate - 0.2) * 0.8)
        assert_equal(mysql.selectUserInfoSql('single_money', self.ceoUid, money_type='money_cash'),
    ...
    def test_03_defendPayCustomRate_25(self, des='个人守护打赏自定义分成:25'):
        ...
        mysql.updateUserMoneyClearSql(self.customUid, self.ceoUid)
        mysql.checkUserBroker(self.customUid, bid=self.ceoUid)  # bid=105002314 被打赏者加入工会
        mysql.checkBrokerUserRate(self.customUid, self.ceoUid, rate=25)  # config.bbc_broker_user_rate 设置分成比
    ...
    def test_04_liveRoomPayCustomRate_70(self, des='直播公会房间打赏自定义分成比:70'):
        """
        用例描述：
        ...
        assert_body(res['body'], 'success', 1, reason(des, res))
        assert_equal(mysql.selectUserInfoSql('sum_money', config.payUid), 0)
        assert_equal(mysql.selectUserInfoSql('single_money',
                                                self.pack_cal_uid,
    ...
    def test_05_liveChatPayCustomRate_0(self, des='直播公会私聊打赏自定义分成比:0'):
        """
        用例描述：
        ...
        """
        mysql.updateMoneySql(config.payUid, money=30, money_cash=3000, money_cash_b=70)
        mysql.updateUserMoneyClearSql(self.pack_cal_uid, self.ceoUid)
        mysql.checkBrokerUserRate(self.pack_cal_uid, self.ceoUid, rate=0)
        data = encodeData(payType='chat-gift',
                          uid=self.pack_cal_uid)
        res = post_request_session(config.pay_url, data)
        assert_code(res['code'])
        assert_body(res['body'], 'success', 1, reason(des, res))
        assert_equal(mysql.selectUserInfoSql('sum_money', config.payUid), 2100)
        assert_equal(mysql.selectUserInfoSql('single_money',
                                                self.pack_cal_uid,
                                                money_type='money_cash'), 0)
        assert_equal(mysql.selectUserInfoSql('sum_money',
                                                self.pack_cal_uid), 0)
        assert_equal(mysql.selectUserInfoSql('single_money',
                                                self.ceoUid,
