...
class TestPayCreate(unittest.TestCase):
    ...
    def test_01_shopPayChangeMoney(self, des='商城购买单个道具场景', cid=329):
        """
        用例描述：
        验证商城购买道具逻辑
        脚本步骤：
        1.构造购买者数据 （更新xs_user_money和xs_user_commodity）
        2.商城内购买礼物道具*1 (cid:329是礼物四叶草)
        3.校验接口状态和返回值数据
    ...
    def test_02_shopPayChangeBuyMore(self, des='商城购买n个道具场景', cid=340):
        """
        用例描述：
        验证商城购买多个道具场景
        脚本步骤：
    ...
    def test_03_shopGiftToUser(self, des='打赏背包内物品场景', bag_gift_cid=340):
        """
        用例描述：
    ...
    def test_04_shopGiftToUserNoEnough(self, des='打赏背包物品但物品不足抵扣的场景', bag_gift_cid=340):
        """
        用例描述：
        验证商城购买的道具赠送时不足的情况
        脚本步骤：
