...
class TestPayCreate(unittest.TestCase):
    ...
    def test_01_shopPayChangeMoney(self, des='商城购买单个道具场景'):
        ...
        5.检查背包内物品
        """
        mysql.updateMoneySql(config.payUid, money_cash=100, money_cash_b=100)
        ...
        assert_equal(mysql.selectUserInfoSql('sum_commodity', config.payUid), 1)
    ...
    def test_02_shopPayChangeBuyMore(self, des='商城购买n个道具场景'):
        ...
        5.检查背包内物品：10
        """
        mysql.updateMoneySql(config.payUid, money=1000, money_cash=100000, money_cash_b=1000, money_b=1000)
        data = encodeData(payType='shop-buy',
        ...
        assert_code(res['code'])
        assert_body(res['body'], 'success', 1, reason(des, res))
        assert_equal(mysql.selectUserInfoSql('sum_money', config.payUid), 4000)
        assert_equal(mysql.selectUserInfoSql('num_commodity', config.payUid,
    ...
    def test_03_shopGiftToUser(self, des='打赏背包内物品场景'):
        ...
        data = encodeData(rid=config.live_role['auto_rid'],
        ...
        assert_equal(mysql.selectUserInfoSql('sum_money', config.rewardUid), 6138)
        case_list[des] = result
