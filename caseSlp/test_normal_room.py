...
class TestPayCreate(unittest.TestCase):
	...
	def test_01_businessPayGiftToGs(self, des='商业房-直播,礼物打赏普通用户到账60%(mcb)'):
		"""
		用例描述：
	...
	def test_02_businessChatgiftToGs(self, des='商业房-直播,房间私聊打赏普通用户到账60%(mcb)'):
		"""
		用例描述：
	...
	def test_03_businessPackageToGs(self, des='商业房-直播,背包礼物打赏普通用户到账60%(mcb)'):
		"""
		用例描述：
	...
	def test_04_businessPayGiftToGs(self, des='商业房-非直播,礼物打赏普通用户到账60%(mc)'):
		"""
		用例描述：
		...
		assert_code(res['code'])
		assert_body(res['body'], 'success', 1, reason(des, res))
		assert_equal(mysql.selectUserInfoSql('single_money', gs_B_uid),
		             giftId['69']['price'] * default_num * rates['normal']['default'])
		assert_equal(mysql.selectUserInfoSql('sum_money', gs_B_uid), giftId['69']['price'] * default_num * rates['normal']['default'])
		assert_equal(mysql.selectUserInfoSql('sum_money', payUid), default_money - giftId['69']['price'] * default_num)
		case_list[des] = result

	def test_05_businessChatgiftToGs(self, des='商业房-非直播,房间私聊打赏普通用户到账60%(mc)'):
		"""
		用例描述：
	...
	def test_06_businessPackageToGs(self, des='商业房-非直播,背包礼物打赏普通用户到账60%(mc)'):
		"""
		用例描述：
	...
	def test_07_businessPayGiftToGs(self, des='个人房-直播,礼物打赏普通用户到账60%(mc)'):
		"""
		用例描述：
	...
	def test_08_businessChatgiftToGs(self, des='个人房-直播,房间私聊打赏普通用户到账60%(mc)'):
		"""
		用例描述：
	...
	def test_09_businessPackageToGs(self, des='个人房-直播,背包礼物打赏普通用户到账60%(mc)'):
		"""
		用例描述：
		个人房-直播,背包礼物打赏普通用户到账60%(mcb)
		脚本步骤：
		1.构造打赏者和被打赏者数据
		2.房间打赏礼物（打赏100分）
		3.校验接口状态和返回值数据
