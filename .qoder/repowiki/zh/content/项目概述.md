# 项目概述

<cite>
**本文档引用的文件**
- [README.md](file://README.md)
- [requirements.txt](file://requirements.txt)
- [run_all_case.py](file://run_all_case.py)
- [common/Config.py](file://common/Config.py)
- [common/Consts.py](file://common/Consts.py)
- [common/Request.py](file://common/Request.py)
- [common/basicData.py](file://common/basicData.py)
- [common/Session.py](file://common/Session.py)
- [common/conMysql.py](file://common/conMysql.py)
- [common/Assert.py](file://common/Assert.py)
- [case/test_pay_shopBuy.py](file://case/test_pay_shopBuy.py)
- [case/test_pay_openBox.py](file://case/test_pay_openBox.py)
- [caseOversea/test_pt_shopBuy.py](file://caseOversea/test_pt_shopBuy.py)
- [caseOversea/test_pt_openBox.py](file://caseOversea/test_pt_openBox.py)
- [caseSlp/config.py](file://caseSlp/config.py)
- [caseSlp/tools.py](file://caseSlp/tools.py)
- [Robot.py](file://Robot.py)
</cite>

## 目录
1. [引言](#引言)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构总览](#架构总览)
5. [详细组件分析](#详细组件分析)
6. [依赖分析](#依赖分析)
7. [性能考虑](#性能考虑)
8. [故障排查指南](#故障排查指南)
9. [结论](#结论)
10. [附录](#附录)

## 引言
本项目为QA支付测试自动化框架，面向多平台、多支付场景的自动化回归与冒烟测试。项目覆盖以下平台与场景：
- 平台：BanBan（国内）、PT海外版（多语言/多大区）、不夜星球（SLP）
- 支付场景：房间打赏、私聊打赏、盲盒开箱、商城购买、守护/房间守护、标题购买、联合/家族房等
- 技术栈：Python + pytest + requests + PyMySQL + GitPython + Slack机器人通知

项目通过统一的请求封装、参数编码、数据库前置/后置处理、断言封装与报告输出，形成可扩展、可维护的测试体系，支持持续集成与自动化执行。

## 项目结构
项目采用按平台与功能分层的组织方式：
- 根目录：入口脚本、运行器、依赖清单、README
- common：通用能力（配置、请求、会话、断言、数据库操作、日志、报告等）
- case：国内平台（BanBan）相关用例
- caseOversea：PT海外版相关用例
- caseSlp：不夜星球（SLP）相关用例
- caseStarify：Starify相关用例（当前注释未启用）
- others：辅助脚本与配置
- probabilityTest：概率玩法测试（盲盒、转盘等）

```mermaid
graph TB
A["根目录"] --> B["common<br/>通用模块"]
A --> C["case<br/>国内平台用例"]
A --> D["caseOversea<br/>PT海外版用例"]
A --> E["caseSlp<br/>不夜星球用例"]
A --> F["caseStarify<br/>Starify用例"]
A --> G["others<br/>辅助脚本"]
A --> H["probabilityTest<br/>概率玩法测试"]
A --> I["run_all_case.py<br/>执行入口"]
A --> J["requirements.txt<br/>依赖清单"]
A --> K["README.md<br/>项目说明"]
```

**图表来源**
- [run_all_case.py:126-147](file://run_all_case.py#L126-L147)
- [README.md:31-37](file://README.md#L31-L37)

**章节来源**
- [README.md:1-38](file://README.md#L1-L38)
- [requirements.txt:1-85](file://requirements.txt#L1-L85)

## 核心组件
- 配置中心：集中管理各平台域名、登录URL、用户/房间/礼物配置、代码分支等
- 请求封装：统一HTTP请求、Header注入、Token读取、响应解析与耗时统计
- 数据编码：按支付场景构造参数（房间打赏、私聊打赏、商城购买、盲盒等）
- 会话管理：登录态获取与持久化（Token写入/读取）
- 数据库操作：前置/后置数据准备与清理、余额/背包/守护等查询
- 断言封装：统一断言方法（状态码、返回体字段、数值范围、文本包含等）
- 执行调度：按平台选择用例目录、自动发现、并发控制、失败重试、通知机器人
- 报告与通知：用例结果汇总、失败原因收集、Slack/微信机器人推送

**章节来源**
- [common/Config.py:6-133](file://common/Config.py#L6-L133)
- [common/Request.py:17-59](file://common/Request.py#L17-L59)
- [common/basicData.py:8-325](file://common/basicData.py#L8-L325)
- [common/Session.py:13-200](file://common/Session.py#L13-L200)
- [common/conMysql.py:8-530](file://common/conMysql.py#L8-L530)
- [common/Assert.py:11-96](file://common/Assert.py#L11-L96)
- [run_all_case.py:12-124](file://run_all_case.py#L12-L124)

## 架构总览
系统采用分层架构与模块化设计：
- 表现层：pytest用例（按平台划分）
- 控制层：执行入口与调度（run_all_case.py）
- 业务层：请求封装、参数编码、断言封装
- 数据访问层：数据库操作（PyMySQL）
- 通知层：Slack/微信机器人

```mermaid
graph TB
subgraph "表现层"
T1["pytest用例<br/>case/*, caseOversea/*, caseSlp/*"]
end
subgraph "控制层"
R["run_all_case.py<br/>平台选择/用例发现/执行/通知"]
end
subgraph "业务层"
REQ["Request.py<br/>HTTP请求封装"]
BD["basicData.py<br/>参数编码"]
AS["Assert.py<br/>断言封装"]
SESS["Session.py<br/>登录态管理"]
end
subgraph "数据访问层"
DB["conMysql.py<br/>数据库操作"]
CFG["Config.py<br/>配置中心"]
end
subgraph "通知层"
ROBOT["Robot.py<br/>Slack/微信通知"]
end
T1 --> REQ
T1 --> BD
T1 --> AS
T1 --> DB
T1 --> CFG
R --> T1
R --> ROBOT
REQ --> SESS
REQ --> DB
```

**图表来源**
- [run_all_case.py:126-147](file://run_all_case.py#L126-L147)
- [common/Request.py:17-59](file://common/Request.py#L17-L59)
- [common/basicData.py:8-325](file://common/basicData.py#L8-L325)
- [common/Assert.py:11-96](file://common/Assert.py#L11-L96)
- [common/Session.py:13-200](file://common/Session.py#L13-L200)
- [common/conMysql.py:8-530](file://common/conMysql.py#L8-L530)
- [common/Config.py:6-133](file://common/Config.py#L6-L133)
- [Robot.py:6-34](file://Robot.py#L6-L34)

## 详细组件分析

### 组件A：执行入口与调度（run_all_case.py）
- 功能：根据主机节点选择平台，自动拉取代码，发现并执行对应目录下的测试用例，统计结果并通过机器人推送
- 关键流程：
  - 平台识别与分支判断
  - 自动拉取代码与更新时间戳
  - 用例发现（unittest.defaultTestLoader.discover）
  - 结果统计与失败原因收集
  - 通知机器人（Slack/微信）

```mermaid
sequenceDiagram
participant Runner as "run_all_case.py"
participant Git as "autoGitPull"
participant Loader as "unittest加载器"
participant Cases as "具体用例"
participant Robot as "Robot.py"
Runner->>Runner : 选择平台与用例目录
Runner->>Git : 拉取代码/更新时间戳
Git-->>Runner : 拉取结果
Runner->>Loader : discover(用例目录)
Loader-->>Runner : 测试套件
Runner->>Cases : 执行用例
Cases-->>Runner : 结果与失败原因
Runner->>Robot : 推送成功/失败通知
```

**图表来源**
- [run_all_case.py:12-124](file://run_all_case.py#L12-L124)
- [run_all_case.py:126-147](file://run_all_case.py#L126-L147)
- [Robot.py:6-34](file://Robot.py#L6-L34)

**章节来源**
- [run_all_case.py:12-124](file://run_all_case.py#L12-L124)
- [run_all_case.py:126-147](file://run_all_case.py#L126-L147)

### 组件B：请求封装（common/Request.py）
- 功能：封装POST请求，统一Header（含User-Agent、Content-Type、Connection、user-token），支持HTTPS、异常处理、响应解析与耗时统计
- 使用：各平台用例通过该模块发起支付请求

```mermaid
flowchart TD
Start(["进入请求封装"]) --> Warn["禁用证书警告"]
Warn --> Header["构建Header<br/>User-Agent/Content-Type/Connection/user-token"]
Header --> UrlCheck{"URL是否以https://开头"}
UrlCheck --> |否| FixUrl["补全为https://前缀"]
UrlCheck --> |是| Post
FixUrl --> Post["发送POST请求"]
Post --> Parse["解析响应JSON/状态码/耗时"]
Parse --> Return["返回响应字典"]
```

**图表来源**
- [common/Request.py:17-59](file://common/Request.py#L17-L59)

**章节来源**
- [common/Request.py:17-59](file://common/Request.py#L17-L59)

### 组件C：参数编码（common/basicData.py）
- 功能：按支付场景构造请求参数（房间打赏、私聊打赏、商城购买、盲盒、守护、标题等），支持多用户、多礼物、多数量组合
- 场景覆盖：
  - 国内平台：房间打赏、私聊打赏、商城购买、盲盒开箱、守护升级/解除等
  - PT海外版：房间打赏、私聊打赏、商城购买（金豆/钻石）、盲盒、转盘、星球抽奖等
  - SLP：房间打赏、商城购买、守护、箱子等

```mermaid
flowchart TD
A["输入：payType/money/rid/uid/giftId/num/..."] --> B{"payType类型"}
B --> |package| P1["构造房间打赏参数"]
B --> |chat-gift| P2["构造私聊打赏参数"]
B --> |shop-buy| P3["构造商城购买参数"]
B --> |shop-buy-box| P4["构造盲盒开箱参数"]
B --> |defend*/title| P5["构造守护/标题参数"]
P1 --> Encode["统一URL编码"]
P2 --> Encode
P3 --> Encode
P4 --> Encode
P5 --> Encode
Encode --> Out["返回编码后的请求参数"]
```

**图表来源**
- [common/basicData.py:8-325](file://common/basicData.py#L8-L325)
- [common/basicData.py:327-566](file://common/basicData.py#L327-L566)

**章节来源**
- [common/basicData.py:8-325](file://common/basicData.py#L8-L325)
- [common/basicData.py:327-566](file://common/basicData.py#L327-L566)

### 组件D：会话与登录（common/Session.py）
- 功能：按环境获取登录Token，支持默认方案与备选方案（数据库取token），Token持久化到本地文件
- 平台适配：国内、PT海外版、不夜星球等

```mermaid
sequenceDiagram
participant Case as "测试用例"
participant Sess as "Session.py"
participant Yaml as "Basic.yml"
participant Api as "登录接口"
participant Token as "Token文件"
Case->>Sess : getSession(env)
Sess->>Yaml : 读取Header/Params/Body
Sess->>Api : 发起登录请求
Api-->>Sess : 返回token/uid
Sess->>Token : 写入Token
Sess-->>Case : 返回token/uid
```

**图表来源**
- [common/Session.py:19-163](file://common/Session.py#L19-L163)

**章节来源**
- [common/Session.py:19-163](file://common/Session.py#L19-L163)

### 组件E：数据库操作（common/conMysql.py）
- 功能：提供账户余额、背包、守护、消费记录等查询与更新；支持批量清理、插入、校验
- 用法：用例执行前后进行数据准备与清理，确保测试隔离性

**章节来源**
- [common/conMysql.py:28-204](file://common/conMysql.py#L28-L204)
- [common/conMysql.py:206-273](file://common/conMysql.py#L206-L273)
- [common/conMysql.py:275-322](file://common/conMysql.py#L275-L322)
- [common/conMysql.py:324-530](file://common/conMysql.py#L324-L530)

### 组件F：断言封装（common/Assert.py）
- 功能：统一断言方法（状态码、返回体字段、长度、相等、范围、文本包含），失败时收集原因

**章节来源**
- [common/Assert.py:11-96](file://common/Assert.py#L11-L96)

### 组件G：平台配置（common/Config.py）
- 功能：集中管理各平台域名、登录URL、用户/房间/礼物ID、代码分支等

**章节来源**
- [common/Config.py:6-133](file://common/Config.py#L6-L133)

### 组件H：全局常量（common/Consts.py）
- 功能：全局用例列表、失败原因列表、时间戳、并发计数等

**章节来源**
- [common/Consts.py:4-17](file://common/Consts.py#L4-L17)

### 组件I：通知机器人（Robot.py）
- 功能：支持多种模式（成功/失败/Markdown/Slack/Slack_PT），向Slack/微信推送测试结果

**章节来源**
- [Robot.py:6-34](file://Robot.py#L6-L34)
- [Robot.py:108-133](file://Robot.py#L108-L133)

### 组件J：SLP专用工具（caseSlp/config.py, caseSlp/tools.py）
- 功能：SLP平台用户、房间、礼物、守护、分成比例等配置；签名生成、数值处理等工具

**章节来源**
- [caseSlp/config.py:1-263](file://caseSlp/config.py#L1-L263)
- [caseSlp/tools.py:23-47](file://caseSlp/tools.py#L23-L47)

## 依赖分析
项目依赖以pytest为核心，结合requests、PyMySQL、GitPython等第三方库，形成稳定的测试生态。

```mermaid
graph TB
PY["pytest"] --> RUN["run_all_case.py"]
REQ["requests"] --> REQMOD["common/Request.py"]
MYSQL["PyMySQL"] --> DBMOD["common/conMysql.py"]
GIT["GitPython"] --> RUN
SLACK["Slack机器人"] --> ROBOT["Robot.py"]
YAML["PyYAML"] --> SESS["common/Session.py"]
```

**图表来源**
- [requirements.txt:60-66](file://requirements.txt#L60-L66)
- [requirements.txt:54-54](file://requirements.txt#L54-L54)
- [requirements.txt:25-25](file://requirements.txt#L25-L25)
- [requirements.txt:65-65](file://requirements.txt#L65-L65)
- [requirements.txt:64-64](file://requirements.txt#L64-L64)

**章节来源**
- [requirements.txt:1-85](file://requirements.txt#L1-L85)

## 性能考虑
- 请求超时与重试：部分断言对RPC接口延迟做了等待，避免误判
- 日志与耗时：请求封装中记录毫秒级耗时，便于定位慢接口
- 并发与资源：执行入口提供并发计数变量，便于后续扩展
- 数据准备：数据库操作采用批量清理与插入，减少重复SQL

**章节来源**
- [common/Assert.py:17-18](file://common/Assert.py#L17-L18)
- [common/Request.py:48-58](file://common/Request.py#L48-L58)
- [common/Consts.py:15-16](file://common/Consts.py#L15-L16)

## 故障排查指南
- 登录失败：检查Basic.yml中的Header/Params/Body是否正确，确认Session写入Token是否成功
- 数据库异常：确认连接参数、库名、表名是否匹配，关注事务回滚与提交
- 断言失败：查看失败原因收集列表，核对期望值与实际返回体
- 通知失败：检查机器人URL配置，确认网络可达性
- 用例执行异常：查看执行入口的日志记录，确认平台选择与用例发现逻辑

**章节来源**
- [common/Session.py:50-67](file://common/Session.py#L50-L67)
- [common/conMysql.py:344-360](file://common/conMysql.py#L344-L360)
- [common/Assert.py:23-25](file://common/Assert.py#L23-L25)
- [Robot.py:36-43](file://Robot.py#L36-L43)
- [run_all_case.py:122-124](file://run_all_case.py#L122-L124)

## 结论
本项目通过统一的请求封装、参数编码、数据库操作与断言封装，实现了多平台、多场景的支付测试自动化。其模块化与分层设计便于扩展与维护，配合Slack通知与Git自动拉取，满足持续集成与快速反馈的需求。建议后续完善Starify平台用例、引入更细粒度的并发控制与报告可视化。

## 附录

### 支付场景与用例映射
- 国内平台（BanBan）
  - 商城购买：[case/test_pay_shopBuy.py:21-124](file://case/test_pay_shopBuy.py#L21-L124)
  - 盲盒开箱：[case/test_pay_openBox.py:15-124](file://case/test_pay_openBox.py#L15-L124)
- PT海外版
  - 商城购买（金豆/钻石）：[caseOversea/test_pt_shopBuy.py:13-58](file://caseOversea/test_pt_shopBuy.py#L13-L58)
  - 盲盒开箱：[caseOversea/test_pt_openBox.py:23-133](file://caseOversea/test_pt_openBox.py#L23-L133)
- 不夜星球（SLP）
  - 配置与工具：[caseSlp/config.py:1-263](file://caseSlp/config.py#L1-L263)、[caseSlp/tools.py:23-47](file://caseSlp/tools.py#L23-L47)

**章节来源**
- [case/test_pay_shopBuy.py:21-124](file://case/test_pay_shopBuy.py#L21-L124)
- [case/test_pay_openBox.py:15-124](file://case/test_pay_openBox.py#L15-L124)
- [caseOversea/test_pt_shopBuy.py:13-58](file://caseOversea/test_pt_shopBuy.py#L13-L58)
- [caseOversea/test_pt_openBox.py:23-133](file://caseOversea/test_pt_openBox.py#L23-L133)
- [caseSlp/config.py:1-263](file://caseSlp/config.py#L1-L263)
- [caseSlp/tools.py:23-47](file://caseSlp/tools.py#L23-L47)