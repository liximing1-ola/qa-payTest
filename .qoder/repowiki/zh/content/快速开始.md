...
## 简介
本指南面向首次接触 QA 支付测试自动化项目的新人，目标是帮助你在最短时间内完成环境准备、配置与执行第一个测试用例，并掌握批量执行、报告查看与常见问题排查的方法。项目基于 Python 与 pytest 框架，通过统一的配置中心、请求封装与断言模块，覆盖多应用（伴伴、PT、SLP 等）的支付场景。
...
TESTCON["testConcurrent.py<br/>并发测试入口"]
RUN1 --> CASE1
RUN1 --> CASE2
RUN1 --> CASE3
RUN2 --> CASE5
...
participant Logs as "日志(Logs.py)"
...
- 依赖安装
  ...
  - 参考：[requirements.txt:1-91](file://requirements.txt#L1-L91)
- 项目说明
...
- 并发测试入口
  - 支持高并发场景的测试执行
  - 参考：[testConcurrent.py](file://testConcurrent.py)
...
- 日志文件
  - 症状：无法查看测试结果
  - 处理：检查日志目录下的 caseResult.log、failCase.log 等文件
  - 参考：[common/Logs.py:23-84](file://common/Logs.py#L23-L84)
...
- [common/Logs.py:23-84](file://common/Logs.py#L23-L84)
