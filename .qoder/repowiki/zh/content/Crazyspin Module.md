...
## 更新摘要
**所做更改**
- 增强了类型注解，为所有方法参数和返回值添加了完整的类型提示
- 改进了测试方法参数签名，增加了详细的docstring说明
- 标准化了变量命名，将pt_payUid改为app_payUid以保持一致性
- 增加了详细的docstring说明，提升了代码可读性和维护性
...
该模块基于Python 3开发，采用面向对象的设计模式，通过静态方法提供简洁易用的API接口，便于在测试用例中集成和使用。最新版本增强了类型注解和文档说明，提升了代码质量和可维护性。
...
- [test_pt_crazySpin.py:1-97](file://caseOversea/test_pt_crazySpin.py#L1-L97)
...
- `spin_buy_url(uid: int) -> str`: 获取购买URL，用于购买欢乐券
- `spin_play_url(uid: int) -> str`: 获取抽奖URL，用于进行转盘抽奖
- `get_turntable_list(rid: int, uid: int, token_name: str = 'dev') -> requests.Response`: 获取转盘列表
- `get_turntable_horn(uid: int, token_name: str = 'dev') -> requests.Response`: 获取转盘喇叭信息
...
+static _build_url(endpoint : str, params : Dict[str, Any]) -> str
+static _build_headers(token_name : str = 'dev') -> Dict[str, str]
+static spin_buy_url(uid : int) -> str
+static spin_play_url(uid : int) -> str
+static get_turntable_list(rid : int, uid : int, token_name : str = 'dev') -> requests.Response
+static get_turntable_horn(uid : int, token_name : str = 'dev') -> requests.Response
}
class Config {
+str app_host
+int app_payUid
...
class Session {
+static checkUserToken(operate : str, app_name : str, token : str, uid : Optional[int]) -> str
}
class requests {
+get(url : str, params : Dict[str, Any], headers : Dict[str, str]) -> Response
+post(url : str, data : Any, headers : Dict[str, str]) -> Response
}
CrazySpin --> Config : "使用"
...
Test->>CS : 调用spin_buy_url(uid : int)
CS->>CS : 构建参数
...
**章节来源**
- [Crazyspin.py:36-152](file://common/Crazyspin.py#L36-L152)
- [test_pt_crazySpin.py:14-97](file://caseOversea/test_pt_crazySpin.py#L14-L97)
...
| 参数名 | 默认值 | 类型 | 用途 |
|--------|--------|------|------|
| `package` | `com.imbb.oversea.android` | str | 应用包名标识 |
| `_ipv` | `0` | str | IP版本标识 |
| `_platform` | `android` | str | 平台类型 |
| `_model` | `Pixel 3a` | str | 设备型号 |
| `_abi` | `arm64-v8a` | str | CPU架构 |
| `format` | `json` | str | 响应格式 |
...
- [test_pt_crazySpin.py:42-97](file://caseOversea/test_pt_crazySpin.py#L42-L97)
...
**更新** 最新版本增强了类型注解、改进了测试方法参数签名、标准化了变量命名（pt_payUid → app_payUid）、增加了详细的docstring说明，显著提升了代码质量和可维护性。