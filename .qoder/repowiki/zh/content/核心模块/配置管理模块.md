...
```mermaid
graph TB
Config["Config类<br/>集中配置"] --> BasicYml["Basic.yml<br/>基础运行配置"]
...
- [common/paramsYaml.py:1-31](file://common/paramsYaml.py#L1-L31)
...
## 核心组件
- Config类：统一承载应用信息、代码路径、应用映射、服务器标识、支付接口、用户/角色/礼物等配置，并提供派生字段（如支付URL、登录URL等）。该类以字典形式组织配置，便于集中访问与扩展。
...
- [common/paramsYaml.py:8-31](file://common/paramsYaml.py#L8-L31)
...
- 集中式配置：Config类集中存放所有静态配置，便于统一维护与版本控制。
...
### Config类设计与多环境配置机制
- 应用信息配置（appInfo）：集中管理各应用的域名前缀，用于拼接支付、登录等接口URL。
- 代码路径配置（codeInfo）：记录各应用的代码仓库路径与分支，用于自动化拉取与通知。
- 应用名称映射（appName）：将人类可读的应用名映射到内部appName键，供入口与会话模块使用。
- 服务器标识（linux_node）：将主机节点名映射到唯一标识，用于入口脚本的环境选择与业务差异处理。
- 支付接口配置（pay_url、slp_pay_url）：基于appInfo动态拼接，确保不同环境下的URL一致性。
- 用户配置（bb_user、pt_user）：存储打赏者、被奖励者、公会成员等UID集合，支持业务层直接引用。
- 角色配置（live_role）：定义直播间、商业房等角色的UID，支撑分账与房间场景。
- 礼物配置（giftId、pt_giftId）：定义礼物ID与价格映射，支撑打赏与商城购买等场景。
...
participant Config as "Config.appName/linux_node"
participant Session as "Session.getSession"
participant Token as "本地Token文件"
OS->>Runner : 获取主机节点
Runner->>Config : 查询appName映射
...
```mermaid
flowchart TD
Start(["业务请求"]) --> LoadConfig["从Config加载房间/用户/礼物配置"]
...
章节来源
- [common/paramsYaml.py:15-31](file://common/paramsYaml.py#L15-L31)
- [common/Session.py:45-67](file://common/Session.py#L45-L67)
- [run_all_case.py:150-159](file://run_all_case.py#L150-L159)
- [common/Config.py:6-133](file://common/Config.py#L6-L133)

## 结论
配置管理模块通过Config类实现了集中化、可扩展的配置体系，结合YAML动态加载与环境适配，有效支撑多应用、多环境的自动化测试与业务流程。建议在现有基础上进一步引入配置校验与热更新能力，以提升系统的可观测性与运维效率。

## 附录

### 配置扩展与最佳实践
- 新增配置项：在Config类中添加字段，并在必要处补充派生字段；确保业务层通过Config统一访问。
