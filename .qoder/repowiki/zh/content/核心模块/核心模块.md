# 核心模块

<cite>
**本文引用的文件**
- [Config.py](file://common/Config.py)
- [Request.py](file://common/Request.py)
- [Session.py](file://common/Session.py)
- [conMysql.py](file://common/conMysql.py)
- [Assert.py](file://common/Assert.py)
- [method.py](file://common/method.py)
- [Consts.py](file://common/Consts.py)
- [getToken.py](file://common/getToken.py)
- [paramsYaml.py](file://common/paramsYaml.py)
- [Logs.py](file://common/Logs.py)
- [Basic.yml](file://common/Basic.yml)
- [run_all_case.py](file://run_all_case.py)
- [test_pay_bean.py](file://case/test_pay_bean.py)
- [test_pay_business.py](file://case/test_pay_business.py)
</cite>

## 更新摘要
**变更内容**
- 新增了完整的类型注解系统，提升了代码的可维护性和IDE支持
- 重构了配置管理模块，采用dataclass架构提供更好的类型安全
- 改进了错误处理机制，增加了统一的异常包装和日志记录
- 优化了会话管理的环境配置，增强了多应用支持
- 完善了断言模块的功能，增加了RPC延迟补偿机制
- 增强了工具方法的实用性，提供了更多的业务计算功能

## 目录
1. [简介](#简介)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构总览](#架构总览)
5. [详细组件分析](#详细组件分析)
6. [依赖关系分析](#依赖关系分析)
7. [性能考量](#性能考量)
8. [故障排查指南](#故障排查指南)
9. [结论](#结论)
10. [附录](#附录)

## 简介
本技术文档聚焦于QA支付测试自动化项目的核心模块，系统性阐述Config配置管理、Request请求处理、Session会话管理、conMysql数据库访问、Assert断言与method工具方法等模块的设计理念、实现原理与使用方式。文档同时说明模块间的依赖与协作关系，给出典型使用场景与扩展点，并提供性能优化建议与排障指引，帮助读者快速理解并高效扩展该自动化体系。

**更新** 本次更新反映了整个核心模块架构的重大优化，包括配置管理的dataclass重构、工具方法的类型注解增强、数据编码的改进以及新增的错误处理机制。

## 项目结构
项目采用"按职责分层"的组织方式：
- common：核心公共模块（配置、请求、会话、数据库、断言、工具、日志、YAML解析等）
- case/caseOversea/caseSlp：各应用/区域的测试用例集合
- run_all_case.py：统一入口，负责根据运行节点选择应用并执行测试套件
- others：环境与辅助脚本（如PHP配置、环境切换等）

```mermaid
graph TB
subgraph "公共模块(common)"
CFG["Config.py<br/>配置中心(dataclass)"]
REQ["Request.py<br/>HTTP客户端封装"]
SES["Session.py<br/>会话与令牌管理"]
DB["conMysql.py<br/>MySQL数据访问层"]
ASS["Assert.py<br/>断言工具"]
METH["method.py<br/>通用工具方法"]
CONST["Consts.py<br/>全局状态"]
TOK["getToken.py<br/>令牌生成"]
YAML["paramsYaml.py<br/>YAML解析"]
LOG["Logs.py<br/>日志"]
BASIC["Basic.yml<br/>基础参数"]
end
subgraph "测试用例"
CASE1["case/test_pay_bean.py"]
CASE2["case/test_pay_business.py"]
end
RUN["run_all_case.py<br/>统一入口"]
RUN --> CASE1
RUN --> CASE2
CASE1 --> CFG
CASE1 --> REQ
CASE1 --> DB
CASE1 --> ASS
CASE1 --> METH
CASE2 --> CFG
CASE2 --> REQ
CASE2 --> DB
CASE2 --> ASS
CASE2 --> METH
REQ --> SES
SES --> TOK
SES --> YAML
SES --> LOG
DB --> CFG
ASS --> CONST
METH --> CONST
METH --> DB
```

**图表来源**
- [run_all_case.py:155-163](file://run_all_case.py#L155-L163)
- [test_pay_bean.py:1-200](file://case/test_pay_bean.py#L1-L200)
- [test_pay_business.py:1-189](file://case/test_pay_business.py#L1-L189)

**章节来源**
- [run_all_case.py:155-163](file://run_all_case.py#L155-L163)
- [test_pay_bean.py:1-200](file://case/test_pay_bean.py#L1-L200)
- [test_pay_business.py:1-189](file://case/test_pay_business.py#L1-L189)

## 核心组件
本节对各核心模块进行深入剖析，涵盖职责边界、关键接口、数据结构、错误处理与性能特征。

**更新** 新版本引入了完整的类型注解系统和dataclass架构，显著提升了代码质量和开发体验。

- **Config配置管理**
  - **设计理念**：采用dataclass重构的集中式配置中心，提供应用信息、代码路径、服务器标识、测试域名、用户角色、礼物映射、房间类型、支付URL等常量与默认值，支持多应用/多环境配置。
  - **关键点**：通过dataclass维护多应用域名、用户UID映射、礼物ID映射、房间类型映射；提供内网支付接口URL与登录URL；通过BASE_PATH定位资源；支持便捷访问属性。
  - **使用场景**：测试用例直接引用config.xxx常量，避免硬编码；统一管理不同应用的URL与用户数据。

- **Request请求处理**
  - **设计理念**：对requests进行轻量封装，统一Header、Token注入、HTTPS强制、超时控制、响应解析与耗时统计。
  - **关键点**：post_request_session函数统一POST请求流程，自动拼接HTTPS前缀、注入user-token、解析JSON、记录状态码与耗时。
  - **使用场景**：所有支付接口调用均通过该封装发起，确保一致性与可观测性。

- **Session会话管理**
  - **设计理念**：多环境登录策略，优先使用YAML配置的登录参数与Header，失败时回退到数据库查询与令牌生成器生成的Token。
  - **关键点**：getSession根据环境选择登录参数；checkUserToken持久化/读取Token；支持dev、rush、PT、SLP等应用环境；YAML解析来自paramsYaml。
  - **使用场景**：测试前置阶段获取有效Token并写入本地文件，供后续请求复用。

- **conMysql数据库操作**
  - **设计理念**：围绕MySQL的增删改查与业务校验，提供账户余额、背包、公会、房间、守护关系等查询与更新方法。
  - **关键点**：静态方法聚合SQL操作；统一autocommit与commit/rollback；提供checkXsGiftConfig、updateMoneySql、insertBeanSql等常用业务方法。
  - **使用场景**：用例前后置清理与断言数据校验。

- **Assert断言模块**
  - **设计理念**：提供多种断言方法，覆盖状态码、长度、相等、文本包含、字段值断言与区间断言，并统一记录失败原因。
  - **关键点**：断言失败时写入fail_case_reason，便于报告与告警；部分断言考虑跨平台差异（如RPC延迟补偿）。
  - **使用场景**：用例中对HTTP响应与数据库结果进行断言。

- **method工具方法**
  - **设计理念**：提供通用工具函数，包括JSON键遍历、结果统计、失败原因构建、VIP经验值计算、路径检查等。
  - **关键点**：getValue用于统计成功/失败次数；reason系列用于失败原因格式化；checkUserVipExp结合数据库查询与等级映射计算经验值。
  - **使用场景**：用例中数据准备、清理、断言辅助与统计。

**章节来源**
- [Config.py:15-247](file://common/Config.py#L15-L247)
- [Request.py:21-86](file://common/Request.py#L21-L86)
- [Session.py:16-144](file://common/Session.py#L16-L144)
- [conMysql.py:8-530](file://common/conMysql.py#L8-L530)
- [Assert.py:46-167](file://common/Assert.py#L46-L167)
- [method.py:11-146](file://common/method.py#L11-L146)

## 架构总览
整体架构以"配置驱动 + 请求封装 + 会话管理 + 数据访问 + 断言与工具"为核心，测试用例通过统一入口加载并执行，形成闭环。

**更新** 新版本架构更加注重类型安全和错误处理的一致性。

```mermaid
sequenceDiagram
participant Runner as "run_all_case.py"
participant Case as "测试用例"
participant Req as "Request.post_request_session"
participant Sess as "Session.checkUserToken"
participant DB as "conMysql"
participant Ass as "Assert"
participant Meth as "method"
Runner->>Case : 加载测试套件
Case->>DB : 准备/清理测试数据
Case->>Req : 发起支付请求
Req->>Sess : 读取user-token
Sess-->>Req : 返回Token
Req-->>Case : 返回响应(状态码/体/耗时)
Case->>Ass : 断言响应与数据库
Case->>Meth : 统计/失败原因/经验值
Case-->>Runner : 结果汇总
```

**图表来源**
- [run_all_case.py:137-163](file://run_all_case.py#L137-L163)
- [test_pay_bean.py:66-107](file://case/test_pay_bean.py#L66-L107)
- [Request.py:49-86](file://common/Request.py#L49-L86)
- [Session.py:124-144](file://common/Session.py#L124-L144)
- [Assert.py:46-167](file://common/Assert.py#L46-L167)
- [method.py:76-146](file://common/method.py#L76-L146)

## 详细组件分析

### Config配置管理模块
**更新** 采用dataclass重构，提供完整的类型安全和便捷访问属性。

- **设计要点**
  - **dataclass架构**：AppConfig、CodeConfig、AppNameConfig、LinuxNodeConfig等使用dataclass提供类型安全
  - **多应用域名映射**：appInfo维护bb_dev、pt_ali_dev/main、starify、slp、rush等
  - **代码路径与分支**：codeInfo记录PHP/Go路径与分支，便于自动化拉取与报告
  - **应用名称映射**：appName将数字或中文映射到应用标识，便于入口选择
  - **服务器标识**：linux_node区分不同CI节点
  - **测试域名与支付URL**：pt_host、pay_url、slp_pay_url等
  - **用户与礼物映射**：bb_user、live_role、giftId、pt_user、pt_room、pt_giftId等
  - **分成比例**：rate用于商业房分成计算
  - **便捷访问属性**：payUid、rewardUid、masterUid等属性提供简化的访问方式

- **使用方式**
  - 在测试用例中直接引用config.xxx常量，避免硬编码
  - 在入口脚本中根据platform.node()与appName选择用例目录与分支

- **扩展点**
  - 新增应用：在appInfo与appName新增映射；在codeInfo补充路径与分支
  - 新增用户/礼物：在对应字典中添加键值

- **性能与可靠性**
  - dataclass常量读取开销极低；注意避免在热路径频繁重复解析YAML

**章节来源**
- [Config.py:15-247](file://common/Config.py#L15-L247)
- [run_all_case.py:155-163](file://run_all_case.py#L155-L163)

### Request请求处理模块
**更新** 增强了错误处理和类型注解，提供了更完善的请求封装。

- **设计要点**
  - **统一Header**：固定User-Agent、Content-Type、Connection等
  - **Token注入**：从Session读取user-token，保证请求合法性
  - **HTTPS强制**：自动补齐https://前缀，规避协议问题
  - **响应解析**：捕获异常，解析JSON，记录状态码与耗时
  - **类型注解**：完整的参数和返回值类型声明
  - **错误处理**：统一的异常捕获和错误返回

- **使用方式**
  - 在用例中调用post_request_session(url, data, tokenName)发起请求
  - 通过返回字典中的code/body/time_consuming/time_total进行断言与统计

- **扩展点**
  - 支持GET/PUT/DELETE：可在同一封装内扩展方法签名
  - 超时与重试：可引入requests超时参数与指数退避策略

- **性能与可靠性**
  - urllib3禁用警告减少噪声；verify=False便于内网测试，生产需谨慎
  - 响应耗时可用于性能监控与慢请求告警

```mermaid
flowchart TD
Start(["进入post_request_session"]) --> Warn["禁用SSL警告"]
Warn --> BuildHeader["构建Header(含user-token)"]
BuildHeader --> FixProto{"URL是否以https开头?"}
FixProto --> |否| AddProto["补全https://前缀"]
FixProto --> |是| Post["发送POST请求"]
AddProto --> Post
Post --> TryResp{"请求成功?"}
TryResp --> |否| HandleErr["打印异常并返回空字典"]
TryResp --> |是| Parse["解析JSON/提取状态码/耗时"]
Parse --> Return["返回响应字典"]
HandleErr --> Return
```

**图表来源**
- [Request.py:49-86](file://common/Request.py#L49-L86)

**章节来源**
- [Request.py:21-86](file://common/Request.py#L21-L86)

### Session会话管理模块
**更新** 重构了环境配置映射，增强了多应用支持和错误处理。

- **设计要点**
  - **多环境登录**：dev/release/rush/PT/SLP等环境分别读取Basic.yml中的Header、Params与Body
  - **回退策略**：默认方案失败时，通过conMysql与getToken生成Token并写入本地文件
  - **Token持久化**：checkUserToken/checkUserToken_slp按应用与UID写入/读取Token文件
  - **YAML解析**：Yaml.read_yaml根据平台节点选择SafeLoader，兼容不同环境
  - **环境配置映射**：ENV_CONFIGS字典提供统一的环境配置管理
  - **错误处理**：统一的日志记录和异常处理机制

- **使用方式**
  - 在测试前置阶段调用getSession(env)获取token并写入文件
  - Request模块通过Session.checkUserToken读取token注入Header

- **扩展点**
  - 新增环境：在getSession中新增elif分支，读取对应YAML参数
  - 自定义签名：可参考被注释掉的starify实现，扩展签名参数与时间戳

- **性能与可靠性**
  - 登录接口可能不稳定，建议在失败时快速回退至Token生成器
  - Token文件读写需注意并发安全与权限

```mermaid
sequenceDiagram
participant Test as "测试用例"
participant Sess as "Session.getSession"
participant Yaml as "Yaml.read_yaml"
participant Req as "requests.session"
participant Tok as "getToken"
participant FS as "checkUserToken"
Test->>Sess : 传入env
Sess->>Yaml : 读取Header/Params/Body
Sess->>Req : POST登录接口
alt 登录成功
Req-->>Sess : 返回token
Sess->>FS : 写入Token文件
Sess-->>Test : 返回token字典
else 登录失败
Sess->>Tok : 生成Token
Tok-->>Sess : 返回token
Sess->>FS : 写入Token文件
Sess-->>Test : 返回token字典
end
```

**图表来源**
- [Session.py:98-144](file://common/Session.py#L98-L144)
- [paramsYaml.py:28-64](file://common/paramsYaml.py#L28-L64)
- [getToken.py:22-114](file://common/getToken.py#L22-L114)

**章节来源**
- [Session.py:16-144](file://common/Session.py#L16-L144)
- [paramsYaml.py:8-64](file://common/paramsYaml.py#L8-L64)
- [getToken.py:1-114](file://common/getToken.py#L1-L114)

### conMysql数据库操作模块
**更新** 保持原有功能完整性，专注于数据访问层的稳定性和可靠性。

- **设计要点**
  - **连接初始化**：在类变量中完成连接、选择数据库、ping与游标初始化
  - **查询方法**：selectUserInfoSql按accountType返回不同字段或聚合值
  - **更新/删除**：deleteUserAccountSql、updateUserInfoSql、updateMoneySql等
  - **业务辅助**：checkXsGiftConfig、insertBeanSql、insertXsUserCommodity、checkUserBroker等

- **使用方式**
  - 用例setUp/tearDown中调用清理/插入/更新方法
  - 断言阶段通过selectUserInfoSql获取期望值并与实际结果对比

- **扩展点**
  - 新增查询：在selectUserInfoSql中新增accountType分支
  - 新增业务：在类中新增对应SQL方法，遵循统一commit/rollback模式

- **性能与可靠性**
  - autocommit=True简化事务管理；注意批量操作时的sleep与锁竞争
  - SQL异常统一捕获并回滚，避免脏数据

```mermaid
classDiagram
class conMysql {
+db_config
+_dbUrl
+_user
+_password
+_dbName
+_dbPort
+con
+cur
+selectUserInfoSql(accountType, uid, money_type, cid)
+deleteUserAccountSql(tableName, uid)
+updateUserInfoSql(tableName, uid, bid)
+updateMoneySql(uid, money, money_cash, money_cash_b, money_b, gold_coin, money_debts)
+insertBeanSql(uid, money_coupon, cash, cash_lock)
+insertXsUserCommodity(uid, cid, num, state)
+checkXsGiftConfig()
+checkUserBroker(uid, bid)
+checkUserXsMentorLevel(uid, level)
+checkBrokerUserRate(uid, creater, rate)
}
```

**图表来源**
- [conMysql.py:8-530](file://common/conMysql.py#L8-L530)

**章节来源**
- [conMysql.py:8-530](file://common/conMysql.py#L8-L530)

### Assert断言模块
**更新** 增强了错误处理机制，提供了统一的断言包装器和RPC延迟补偿。

- **设计要点**
  - **断言包装器**：@_assert_wrapper统一处理异常和成功返回
  - **RPC延迟补偿**：_delay_for_rpc函数处理非阿里云环境的RPC接口延迟
  - **失败记录**：断言失败时将原因追加到fail_case_reason，便于报告
  - **类型注解**：完整的参数和返回值类型声明
  - **统一异常处理**：所有断言异常都会被包装并记录

- **使用方式**
  - 在用例中链式调用多个断言，确保响应与数据库状态符合预期

- **扩展点**
  - 可新增更细粒度的断言（如JSON Schema校验）
  - 可引入断言失败重试策略（配合Retry装饰器）

- **性能与可靠性**
  - 断言失败快速失败，避免冗余计算；RPC延迟补偿减少误判

**章节来源**
- [Assert.py:16-167](file://common/Assert.py#L16-L167)
- [Consts.py:4-17](file://common/Consts.py#L4-L17)

### method工具方法模块
**更新** 增强了工具方法的实用性和类型安全性。

- **设计要点**
  - **字典转换**：dict_to_slack_fields、dict_to_markdown提供格式化功能
  - **图片获取**：get_image支持多种API源，提供随机图片获取
  - **JSON工具**：is_extend、_get_all_keys递归遍历JSON键，判断字段是否存在
  - **结果处理**：get_value、format_reason、format_reason_slp统计成功/失败、格式化失败原因
  - **路径检查**：check_path检查代码路径是否存在，异常则告警
  - **爵位等级**：TITLE_LEVEL_MAP提供爵位系数映射，calculate_vip_exp计算经验值

- **使用方式**
  - 用例中调用reason构建失败原因，getValue统计结果，checkUserVipExp计算期望值

- **扩展点**
  - 可新增更多业务映射与计算方法
  - 可集成更多第三方通知渠道（如钉钉、企业微信）

- **性能与可靠性**
  - JSON遍历与字符串拼接开销较低；注意避免在高频路径重复构建长字符串

**章节来源**
- [method.py:11-146](file://common/method.py#L11-L146)

## 依赖关系分析
**更新** 依赖关系更加清晰，错误处理机制统一化。

- **模块耦合**
  - Request依赖Session读取Token；Session依赖Yaml与getToken；Logs提供日志能力
  - 测试用例依赖Config、Request、conMysql、Assert、method；run_all_case统一调度
  - 所有模块都依赖Config提供的全局配置

- **外部依赖**
  - requests、urllib3、pymysql、yaml、logging、platform、os、time等

- **循环依赖**
  - 当前模块间无循环导入；断言与工具通过Consts共享全局状态

```mermaid
graph LR
REQ["Request"] --> SES["Session"]
SES --> YAML["paramsYaml"]
SES --> TOK["getToken"]
SES --> LOG["Logs"]
CASE1["test_pay_bean.py"] --> REQ
CASE1 --> DB["conMysql"]
CASE1 --> ASS["Assert"]
CASE1 --> METH["method"]
CASE1 --> CFG["Config"]
CASE2["test_pay_business.py"] --> REQ
CASE2 --> DB
CASE2 --> ASS
CASE2 --> METH
CASE2 --> CFG
RUN["run_all_case.py"] --> CASE1
RUN --> CASE2
CFG --> CONST["Consts"]
ASS --> CONST
METH --> CONST
```

**图表来源**
- [Request.py:7](file://common/Request.py#L7)
- [Session.py:9](file://common/Session.py#L9)
- [paramsYaml.py:5](file://common/paramsYaml.py#L5)
- [getToken.py:10](file://common/getToken.py#L10)
- [Logs.py:13](file://common/Logs.py#L13)
- [test_pay_bean.py:1-9](file://case/test_pay_bean.py#L1-L9)
- [test_pay_business.py:1-9](file://case/test_pay_business.py#L1-L9)
- [run_all_case.py:1-9](file://run_all_case.py#L1-L9)
- [Config.py:10](file://common/Config.py#L10)
- [Assert.py:12](file://common/Assert.py#L12)
- [method.py:6](file://common/method.py#L6)

**章节来源**
- [Request.py:7](file://common/Request.py#L7)
- [Session.py:9](file://common/Session.py#L9)
- [paramsYaml.py:5](file://common/paramsYaml.py#L5)
- [getToken.py:10](file://common/getToken.py#L10)
- [Logs.py:13](file://common/Logs.py#L13)
- [test_pay_bean.py:1-9](file://case/test_pay_bean.py#L1-L9)
- [test_pay_business.py:1-9](file://case/test_pay_business.py#L1-L9)
- [run_all_case.py:1-9](file://run_all_case.py#L1-L9)

## 性能考量
**更新** 性能优化更加注重类型安全和错误处理效率。

- **请求层**
  - 合理设置超时与重试，避免阻塞；对慢接口单独统计耗时
  - 避免在Header中频繁变更，减少不必要的网络往返
  - 利用dataclass的类型检查减少运行时错误

- **会话层**
  - Token缓存与复用，减少重复登录；并发场景注意Token文件读写同步
  - 环境配置映射优化，避免重复解析YAML

- **数据层**
  - 大批量更新/删除时使用事务与批量提交，减少锁竞争
  - 对热点查询建立索引或缓存中间态数据

- **断言与工具**
  - 断言失败快速失败，避免冗余计算；日志按需输出，避免IO瓶颈
  - 类型注解帮助IDE提供更好的代码补全和错误检测

- **并发执行**
  - 使用统一入口run_all_case并行执行用例，注意数据库与文件系统的并发安全

## 故障排查指南
**更新** 故障排查指南更加系统化，涵盖了新的错误处理机制。

- **请求失败**
  - 检查URL协议与Host是否正确；确认Token是否有效；查看Response JSON与状态码
  - 参考：[Request.py:49-86](file://common/Request.py#L49-L86)

- **会话异常**
  - 查看getSession日志与YAML参数；确认登录接口可用性；必要时回退到getToken生成Token
  - 参考：[Session.py:98-144](file://common/Session.py#L98-L144)、[paramsYaml.py:28-64](file://common/paramsYaml.py#L28-L64)、[getToken.py:22-114](file://common/getToken.py#L22-L114)

- **数据库异常**
  - 检查SQL执行日志与异常回滚；确认连接参数与目标库；核对表结构与索引
  - 参考：[conMysql.py:8-530](file://common/conMysql.py#L8-L530)

- **断言失败**
  - 查看fail_case_reason与具体字段值；核对期望值计算逻辑（如VIP经验值）
  - 参考：[Assert.py:16-167](file://common/Assert.py#L16-L167)、[method.py:76-146](file://common/method.py#L76-L146)

- **日志与报告**
  - 使用Logs模块输出到文件与控制台；关注caseResult.log与failCase.log
  - 参考：[Logs.py:50-84](file://common/Logs.py#L50-L84)

- **类型注解问题**
  - 检查dataclass字段的类型声明；确认导入的typing模块正确
  - 参考：[Config.py:11](file://common/Config.py#L11)

**章节来源**
- [Request.py:49-86](file://common/Request.py#L49-L86)
- [Session.py:98-144](file://common/Session.py#L98-L144)
- [paramsYaml.py:28-64](file://common/paramsYaml.py#L28-L64)
- [getToken.py:22-114](file://common/getToken.py#L22-L114)
- [conMysql.py:8-530](file://common/conMysql.py#L8-L530)
- [Assert.py:16-167](file://common/Assert.py#L16-L167)
- [method.py:76-146](file://common/method.py#L76-L146)
- [Logs.py:50-84](file://common/Logs.py#L50-L84)
- [Config.py:11](file://common/Config.py#L11)

## 结论
**更新** 本项目通过清晰的模块划分与职责边界，实现了配置驱动、请求封装、会话管理、数据访问、断言与工具的完整闭环。

本项目通过清晰的模块划分与职责边界，实现了配置驱动、请求封装、会话管理、数据访问、断言与工具的完整闭环。Config提供统一配置，Request与Session保障请求合法性与稳定性，conMysql支撑业务数据校验，Assert与method提升断言与统计能力。

**更新** 新版本架构通过dataclass重构、类型注解增强和统一错误处理机制，显著提升了代码质量、可维护性和开发体验。建议在后续迭代中进一步完善超时与重试、并发安全与日志分级，以增强系统的健壮性与可维护性。

## 附录
- **使用示例（路径引用）**
  - 统一入口执行测试套件：[run_all_case.py:137-163](file://run_all_case.py#L137-L163)
  - 金豆支付用例示例：[test_pay_bean.py:46-200](file://case/test_pay_bean.py#L46-L200)
  - 商业房支付用例示例：[test_pay_business.py:18-98](file://case/test_pay_business.py#L18-L98)

- **配置与参数**
  - 基础参数YAML：[Basic.yml](file://common/Basic.yml)
  - 全局状态：[Consts.py:4-17](file://common/Consts.py#L4-L17)

- **新增功能**
  - dataclass配置管理：提供类型安全的配置访问
  - 统一断言包装器：标准化错误处理和日志记录
  - RPC延迟补偿：处理跨平台差异
  - 增强的类型注解：提升IDE支持和代码质量

**章节来源**
- [run_all_case.py:137-163](file://run_all_case.py#L137-L163)
- [test_pay_bean.py:46-200](file://case/test_pay_bean.py#L46-L200)
- [test_pay_business.py:18-98](file://case/test_pay_business.py#L18-L98)
- [Config.py:15-247](file://common/Config.py#L15-L247)
- [Assert.py:16-167](file://common/Assert.py#L16-L167)
- [Consts.py:4-17](file://common/Consts.py#L4-L17)