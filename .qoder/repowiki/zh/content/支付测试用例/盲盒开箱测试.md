...
- 完善重试机制说明，展示改进的装饰器功能
- 增强测试数据准备和数据库验证的标准流程
- **更新** 背包开箱测试从PT数据编码改为APP数据编码，移除了房间送箱子相关测试方法，更新了用户配置从pt_user到app_user
...
**更新** 本次更新反映了测试用例的重构，包括测试生命周期管理的标准化和测试用例结构的统一化改进，以及背包开箱测试从PT数据编码改为APP数据编码的重要变更。
...
- [test_pt_openBox.py:1-121](file://caseOversea/test_pt_openBox.py#L1-L121)
...
- 数据编码与参数化
  ...
  - **更新** APP数据编码：使用 `encodeAppData` 替代原有的 `encodeData` 进行APP端数据编码
- 数据库访问
...
- [test_pt_openBox.py:23-121](file://caseOversea/test_pt_openBox.py#L23-L121)
...
- 场景覆盖
  ...
  - 房间多人多盲盒（海外）
- 关键差异
  - **更新** 使用APP数据编码：`encodeAppData` 替代原有的 `encodeData` 进行参数编码
  - **更新** 用户配置：使用 `config.app_user` 替代 `config.pt_user`
...
participant Enc as "参数编码(APP)"
participant Req as "请求封装"
participant Srv as "支付服务(海外)"
participant DB as "海外数据库访问"
TC->>DB : 准备用户资产/背包/箱子
TC->>Enc : 生成 pay/create 参数(APP编码)
Enc-->>TC : 返回编码后的数据
TC->>Req : post_request_session(url, data, tokenName='app')
Req->>Srv : 发送 HTTPS 请求
Srv-->>Req : 返回响应
Req-->>TC : 解析响应并返回
...
- 商品配置
  ...
  - **更新** 海外：维护海外礼物 ID 与房间属性，确保盲盒场景正确，使用APP用户配置
- 掉落池与刷新
...
- 组件耦合
  ...
  - **更新** 海外盲盒开箱测试现在使用APP数据编码替代PT数据编码
- 外部依赖
...
## 结论
本测试体系围绕"盲盒开箱"的多场景需求，提供了从参数编码、请求封装、数据库校验到断言与重试的完整链路。结合概率测试脚本，能够有效验证概率算法与随机性。**更新** 新版本测试用例通过标准化的测试生命周期管理和统一的测试结构，进一步提升了测试的稳定性和可维护性。**更新** 特别是在海外盲盒开箱测试中，已完全迁移到APP数据编码模式，使用APP用户配置，确保了测试的一致性和准确性。建议在后续迭代中补充前端动画效果校验，并完善稀有掉落的统计分析与回归报告。

## 附录
- 测试方法速查
  - 国内盲盒开箱：准备铜/银箱子，调用 shop-buy-box，断言余额与背包
  - **更新** 海外盲盒开箱：准备盲盒，调用 shop-buy-box 或 package-more，断言余额与钱包扩展字段，使用APP数据编码
