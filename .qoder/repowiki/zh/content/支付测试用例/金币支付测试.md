...
- 测试生命周期管理：新增setUp()和tearDown()方法，规范测试前后处理
- 重试机制改进：使用@Retry装饰器支持按函数前缀筛选重试用例
...
**更新** 本次重构引入了标准化的测试框架，通过统一的辅助方法提升代码质量和可维护性，新增测试生命周期管理和重试机制。
...
- 重试机制：失败自动重试与环境重置
- 全局变量：用例列表、结果标记、并发统计
- 测试生命周期：setUp()和tearDown()方法管理测试前后状态

**更新** 新增标准化测试框架，通过_prepare_test_data()和_validate_db_state()方法统一数据准备和验证流程，以及测试生命周期管理。
...
**更新** 新增标准化测试框架，通过统一的辅助方法提升测试流程的一致性和可维护性，以及测试生命周期管理。

```mermaid
sequenceDiagram
participant TC as "测试用例"
participant SETUP as "测试前置<br/>setUp()"
...
participant TEARDOWN as "测试后置<br/>tearDown()"
TC->>SETUP : "初始化测试环境"
SETUP-->>TC : "环境准备完成"
TC->>PREP : "准备测试数据"
PREP-->>TC : "数据准备完成"
...
**图表来源**
- [test_pay_coin.py:17-23](file://case/test_pay_coin.py#L17-L23)
...
### 场景十：标准化测试框架详解
- **类名重构**：TestPayCreate重命名为TestPayCoin，体现金币支付的专业化命名
- **测试生命周期管理**：新增setUp()和tearDown()方法，规范测试环境初始化和清理
...
- **测试生命周期**：setUp()和tearDown()方法确保测试环境的一致性，避免跨用例污染
...
## 结论
本测试体系通过参数编码、请求封装、数据库校验与断言封装，覆盖了金币支付的主要场景。结合重试机制与全局变量管理，能够稳定地执行并记录测试结果。**更新** 重构后的标准化测试框架通过统一的辅助方法提升了代码质量和可维护性，新增测试生命周期管理和重试机制，建议后续补充金币退款与更丰富的优惠券叠加场景，以进一步完善金币支付的全链路验证。
