...
- **海外版测试优化**：新增海外版APP支付测试，采用新的编码方法和参数配置
...
subgraph "海外版测试用例"
T3["caseOversea/test_pt_bean.py<br/>APP海外版金豆兑换"]
end
subgraph "公共组件"
C1["common/Config.py<br/>全局配置与常量"]
C2["common/basicData.py<br/>请求参数编码"]
C3["common/conMysql.py<br/>国内数据库操作"]
C4["common/conPtMysql.py<br/>海外版数据库操作"]
C5["common/Assert.py<br/>断言封装"]
C6["common/Request.py<br/>HTTP请求封装"]
C7["common/runFailed.py<br/>失败重试装饰器"]
C8["common/Consts.py<br/>全局状态与统计"]
C9["common/method.py<br/>工具方法与VIP经验计算"]
end
subgraph "运行入口"
...
- [test_pt_bean.py:1-60](file://caseOversea/test_pt_bean.py#L1-L60)
- [Config.py:1-247](file://common/Config.py#L1-L247)
- [basicData.py:1-647](file://common/basicData.py#L1-L647)
- [conMysql.py:1-367](file://common/conMysql.py#L1-L367)
- [conPtMysql.py:1-367](file://common/conPtMysql.py#L1-L367)
- [Assert.py:1-167](file://common/Assert.py#L1-L167)
- [Request.py:1-119](file://common/Request.py#L1-L119)
- [runFailed.py:1-72](file://common/runFailed.py#L1-L72)
- [Consts.py:1-17](file://common/Consts.py#L1-L17)
- [method.py:1-252](file://common/method.py#L1-L252)
...
**章节来源**
- [Config.py:47-247](file://common/Config.py#L47-L247)
- [Request.py:17-119](file://common/Request.py#L17-L119)
- [conMysql.py:27-367](file://common/conMysql.py#L27-L367)
- [conPtMysql.py:27-367](file://common/conPtMysql.py#L27-L367)
- [Assert.py:11-167](file://common/Assert.py#L11-L167)
- [basicData.py:8-647](file://common/basicData.py#L8-L647)
- [runFailed.py:10-72](file://common/runFailed.py#L10-L72)
...
participant DB as "数据库(conMysql/conPtMysql)"
...
- [test_pt_bean.py:27-59](file://caseOversea/test_pt_bean.py#L27-L59)
- [basicData.py:8-647](file://common/basicData.py#L8-L647)
- [Request.py:17-119](file://common/Request.py#L17-L119)
- [conMysql.py:27-367](file://common/conMysql.py#L27-L367)
- [conPtMysql.py:27-367](file://common/conPtMysql.py#L27-L367)
...
### APP海外版金豆兑换测试（test_pt_bean.py）
**更新** 新增海外版APP专项测试，采用优化的编码方法和参数配置

- **测试类与重试装饰器**
  - 使用失败重试装饰器，支持最多3次重试
  - setUpClass：检查礼物配置，确保测试环境一致性
  - 测试方法：test_01_moneyExchangeCoin，验证余额兑换金豆流程

- **编码方法优化**
  - **更新** 从 `encodePtData` 改为 `encodeAppData`，提供更好的类型注解和文档说明
  - 使用 `config.app_payUid` 作为用户ID，区分海外版用户配置

- **测试场景：余额兑换金豆**
  - 设计思路：验证用户余额兑换金豆的完整流程
  - 执行步骤：更新用户余额为300，调用exchange_gold接口，验证兑换结果
  - 预期结果：接口成功返回，账户余额从300变为0，金豆余额从0变为600
  - 数据库验证：使用 `selectUserInfoSql` 方法验证余额变化

```mermaid
flowchart TD
Start(["开始"]) --> Update["更新用户余额<br/>- money: 300<br/>- uid: config.app_payUid"]
Update --> Encode["编码请求参数<br/>- payType: exchange_gold<br/>- 使用encodeAppData"]
Encode --> Send["发送支付请求<br/>- URL: config.app_pay_url<br/>- Token: app"]
Send --> RespOK{"响应成功？"}
RespOK --> |否| Fail["记录失败原因并退出"]
RespOK --> |是| DBCheck["数据库状态验证<br/>- 余额: 0<br/>- 金豆: 600"]
DBCheck --> Done(["结束"])
```

**图表来源**
- [test_pt_bean.py:27-59](file://caseOversea/test_pt_bean.py#L27-L59)
- [basicData.py:573-635](file://common/basicData.py#L573-L635)
- [conPtMysql.py:124-150](file://common/conPtMysql.py#L124-L150)
...
- [basicData.py:161-181](file://common/basicData.py#L161-L181)
- [conMysql.py:27-367](file://common/conMysql.py#L27-L367)
- [Assert.py:11-167](file://common/Assert.py#L11-L167)
...
### 配置中心（Config.py）
**更新** 新增海外版用户配置和APP相关配置

- 支付接口URL：统一内网支付接口地址
- 用户ID：打赏者、被打赏者、GS用户、一代宗师等角色ID
- **APP用户配置**：新增 `AppUserConfig` 类，包含海外版用户ID
- 礼物ID：金豆礼物与钻石礼物的映射
- 分成比例：GS商业房分成比
- **APP配置**：新增 `app_payUid`、`app_pay_url` 等海外版专用配置

**章节来源**
- [Config.py:93-247](file://common/Config.py#L93-L247)
...
### 数据准备与断言（conMysql.py、conPtMysql.py、Assert.py）
**更新** 新增海外版数据库操作支持
...
- **海外版数据库操作**：
  - **新增** `conPtMysql` 模块，提供海外版APP专用数据库操作
  - 支持 `selectUserInfoSql` 方法，使用 `config.pt_payUid` 作为默认用户ID
  - 提供 `updateMoneySql`、`deleteUserAccountSql` 等常用操作
...
- 支持多种payType：
  ...
  - 私聊场景：chat-gift
  - 商店/防御/标题等：对应类型
  - **APP海外版**：新增 `exchange_gold` 场景
- **编码方法优化**：
  - **新增** `encodeAppData` 方法，提供更好的类型注解和文档
  - **更新** `encodePtData` 方法，增强参数验证和错误处理
  - 支持海外版用户ID `config.pt_payUid` 和APP用户ID `config.app_payUid`
...
**章节来源**
- [method.py:229-252](file://common/method.py#L229-L252)
...
TPB2["test_pt_bean.py"] --> CFG
TPB2 --> BDATA
TPB2 --> REQ
TPB2 --> DBPT["conPtMysql.py"]
TPB2 --> ASS
TPB2 --> RETRY
TPB2 --> CONST
TPB2 --> METHOD
...
- **海外版优化**：APP海外版测试使用专门的数据库连接和配置，提升测试效率

**章节来源**
- [Assert.py:17-24](file://common/Assert.py#L17-L24)
- [Request.py:25-32](file://common/Request.py#L25-L32)
- [conMysql.py:350-361](file://common/conMysql.py#L350-L361)
- [conPtMysql.py:26-44](file://common/conPtMysql.py#L26-L44)
...
## 结论
本测试体系围绕金豆支付的多场景构建，通过统一的配置、参数编码、请求封装、数据库准备与断言机制，形成可维护、可扩展的自动化测试方案。结合失败重试与全局状态记录，能够有效提升测试稳定性与可观测性。**新增的海外版APP测试进一步完善了测试覆盖范围，采用优化的编码方法和参数配置，显著提升了代码质量和可维护性**。建议在后续迭代中持续完善边界条件与异常场景覆盖，并保持配置与参数编码的一致性。
...
### 海外版APP测试优化详解
**更新** 新增海外版APP专项测试，采用多项代码优化

- **编码方法优化**
  - 从 `encodePtData` 改为 `encodeAppData`，提供更好的类型注解和文档说明
  - 增强的参数验证和错误处理机制
  - 支持APP用户ID配置

- **数据库操作优化**
  - 新增 `conPtMysql` 模块，提供海外版专用数据库操作
  - 使用 `config.app_payUid` 作为默认用户ID
  - 支持海外版用户配置
...
- [basicData.py:573-635](file://common/basicData.py#L573-L635)
- [conPtMysql.py:124-150](file://common/conPtMysql.py#L124-L150)
- [Config.py:93-120](file://common/Config.py#L93-L120)