...
- 测试生命周期管理：添加 `setUp()` 和 `tearDown()` 方法
- 扩展测试场景：新增多用户场景、特殊房间类型、电台房等场景
- 增强数据验证：标准化数据库状态验证机制
- 完善优惠券测试：涵盖激活券、未激活券、体验券等多种优惠券类型
- **更新** APP数据编码：海外商城购买测试从PT数据编码改为APP数据编码，使用encodeAppData函数
- **更新** 用户配置：从pt_user迁移到app_user配置，使用config.app_payUid等APP用户配置
...
C --> K
D --> L["参数编码(APP)<br/>common/basicData.py(APP)"]
```
...
**更新** 使用APP数据编码函数encodeAppData替代原有的encodePtData，使用APP用户配置config.app_payUid

```mermaid
sequenceDiagram
participant T as "测试用例(APP)"
participant PM as "数据库封装(APP)"
participant E as "参数编码(APP)"
participant R as "请求封装"
participant A as "断言封装"
T->>PM : 准备金豆/钻石余额(app_user)
T->>E : 构造coin-shop-buy/shop-buy参数(encodeAppData)
E-->>T : 返回编码数据
T->>R : post_request_session(app_token)
R-->>T : 返回响应
T->>PM : 查询余额/背包
PM-->>T : 返回结果
T->>A : 断言code/body/equal
...
- **更新** APP用户配置：确认使用config.app_payUid等APP用户配置而非pt_user配置。
...
## 结论
本测试体系围绕"商城购买"核心场景，提供了从参数构造、接口调用、数据库校验到断言反馈的完整闭环。通过统一的配置中心、请求封装与数据库工具，能够稳定复现并验证普通商品、批量购买、积分兑换、打赏抵扣等多类购买流程。**更新** 优惠券支付测试类TestPayCoupon引入了统一的数据准备和数据库验证机制，扩展了多用户场景和特殊房间类型的测试覆盖，显著提升了测试的标准化程度和可维护性。**更新** 海外商城购买测试已从PT数据编码迁移至APP数据编码，使用encodeAppData函数和APP用户配置，进一步统一了测试框架的编码规范。建议在持续集成中定期执行，配合失败重试与机器人通知机制，提升回归效率与问题定位速度。
...
- **更新** APP用户配置：使用config.app_payUid、config.app_testUid等APP用户配置进行测试。
...
- **更新** APP数据编码：使用encodeAppData函数进行APP平台的参数编码。
...
### 测试生命周期管理
**新增** 测试类的生命周期管理改进：
- setUp()：测试前置清理，确保测试环境的一致性
- tearDown()：测试后置清理，恢复测试环境状态
- 统一数据准备：通过`_prepare_test_data()`方法标准化测试数据准备流程
- 统一验证：通过`_validate_db_state()`方法标准化数据库状态验证

**章节来源**
- [test_pay_coupon.py:17-23](file://case/test_pay_coupon.py#L17-L23)
- [test_pay_coupon.py:25-44](file://case/test_pay_coupon.py#L25-L44)
- [test_pay_coupon.py:37-44](file://case/test_pay_coupon.py#L37-L44)

### APP数据编码与用户配置更新
**新增** APP平台商城购买测试的关键变更：
- **APP数据编码**：海外商城购买测试从PT数据编码encodePtData迁移到APP数据编码encodeAppData
- **用户配置**：从pt_user配置迁移到app_user配置，使用config.app_payUid等APP用户标识
- **请求头处理**：APP测试使用token_name='app'参数，区别于国内版的默认token处理
- **配置映射**：APP用户配置位于Config类的AppUserConfig中，提供独立的用户ID集合

**章节来源**
- [test_pt_shopBuy.py:12-57](file://caseOversea/test_pt_shopBuy.py#L12-L57)
