...
## 更新摘要
**所做更改**
- 更新了舰队房测试类名标准化，从 TestPayFleetRoom 改为 TestPayFleetRoom
- 引入了统一的测试生命周期管理框架，包括 setUp 和 tearDown 方法
- 新增了数据准备和验证框架，提供 _prepare_test_data 和 _validate_db_state 方法
...
subgraph "测试用例(case)"
U["test_pay_unionRoom.py<br/>联盟房<br/>TestPayCreate"]
...
V["test_pay_vipRoom.py<br/>VIP房<br/>TestPayCreate"]
C["test_pay_coin.py<br/>金币/余额兑换<br/>TestPayCoin"]
S["test_pay_shopBuy.py<br/>商城购买<br/>TestPayCreate"]
end
U --> CFG
F --> CFG
D --> CFG
...
- [test_pay_fleetRoom.py:12-14](file://case/test_pay_fleetRoom.py#L12-L14)
...
- 用例路径
  ...
  - [test_02_prettyRoomPayBox:73-106](file://case/test_pay_prettyRoom.py#L73-L106)
...
**更新** VIP房测试类名已标准化为 TestPayCreate，采用了统一的测试方法结构，提供了清晰的断言策略和数据库验证方法。
...
**更新** 商城购买测试类名已标准化为 TestPayCreate，采用了统一的测试方法结构和断言策略，提供了清晰的场景化测试流程。
...
```mermaid
graph LR
U["test_pay_unionRoom.py<br/>TestPayCreate"] --> CFG["Config.py"]
U --> DB["conMysql.py"]
U --> REQ["Request.py"]
U --> ASS["Assert.py"]
...
P --> RUN
P --> CONST
V["test_pay_vipRoom.py<br/>TestPayCreate"] --> CFG
V --> DB
V --> REQ
V --> ASS
V --> BASIC
...
C --> RUN
C --> CONST
S["test_pay_shopBuy.py<br/>TestPayCreate"] --> CFG
S --> DB
S --> REQ
S --> ASS
S --> BASIC
