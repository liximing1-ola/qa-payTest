...
**更新** 本测试体系已重构，引入了自动重试机制和标准化的测试生命周期管理，确保测试用例的稳定性和可重复性。
...
- 测试运行器（run_all_case）：统一测试执行入口，支持多应用环境
- 方法工具（method）：提供测试辅助方法，包括失败原因收集和格式化输出

**更新** 新增了测试运行器和方法工具，完善了测试生命周期管理。
...
- [method.py:115-129](file://common/method.py#L115-L129)
...
**更新** 流程中增加了自动重试机制，在测试失败时自动重试指定次数。
...
- **更新** 测试用例结构标准化，使用unittest.TestCase基类和pytest标记
...
- **更新** 引入了完整的自动重试机制，支持最多3次重试

**章节来源**
- [runFailed.py:10-87](file://common/runFailed.py#L10-L87)

### 组件H：测试运行器（run_all_case）
- 功能概述
  - 统一测试执行入口，支持多应用环境（国内、海外、SLP等）
  - 自动发现和执行测试用例
  - 支持测试结果统计和通知
- 使用方式
  - 根据平台选择不同的测试套件
...
### 组件I：方法工具（method）
- 功能概述
  - 提供测试辅助方法，包括失败原因收集和格式化输出
  - 支持测试结果的格式化和机器人通知
- 使用方式
  - 在测试用例中调用reason方法生成详细的失败原因
  - 格式化测试结果用于报告输出
...
**更新** 重构后的测试体系引入了自动重试机制和标准化的测试生命周期管理，显著提升了测试的稳定性和可靠性。通过测试类结构优化、执行顺序控制和失败原因收集机制，确保收益分配算法与房间控制逻辑符合预期。
