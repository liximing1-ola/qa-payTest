...
## 更新摘要
**变更内容**
- 新增测试生命周期管理改进，包括setUp和tearDown方法
- 标准化测试用例结构，统一使用_prepare_test_data和_validate_db_state方法
- 增强重试机制，使用@Retry装饰器实现自动重试
- 优化数据库操作，提供统一的数据准备和验证方法
- 改进测试用例的可维护性和可读性
...
**更新** 本次更新反映了测试用例的重构，包括测试生命周期管理改进和测试用例结构的标准化。
...
**更新** 测试用例现在包含完整的生命周期管理，包括测试数据准备和验证方法的标准化。
...
**更新** 新的架构增加了测试生命周期管理，包括测试数据准备和数据库状态验证的标准化流程。

```mermaid
sequenceDiagram
participant TC as "测试用例"
participant SETUP as "测试数据准备"
...
TC->>SETUP : setUp() - 初始化测试环境
SETUP->>CFG : 读取用户UID/房间ID/礼物ID/分成比例
SETUP->>DB : 准备测试数据(update_money/clear_user_money)
...
REQ-->>TC : 解析响应
TC->>DB : 验证数据库状态(_validate_db_state)
DB-->>TC : 返回数据库结果
TC->>TC : 断言(状态码/返回体/余额/经验)
TC->>SETUP : tearDown() - 清理测试环境
```

**图表来源**
- [test_pay_prettyRoom.py:16-142](file://case/test_pay_prettyRoom.py#L16-L142)
...
**更新** 测试用例现在包含完整的生命周期管理，使用统一的测试方法结构：

测试要点：
- 使用`@Retry(max_n=3)`装饰器实现自动重试机制
- 通过`_prepare_test_data`方法统一准备测试数据
- 通过`_validate_db_state`方法统一验证数据库状态
- 使用标准化的测试方法命名（test_01_*, test_02_*, test_03_*）
- 包含详细的测试步骤注释和预期结果验证

```mermaid
sequenceDiagram
participant TC as "测试用例(TestPayPrettyRoom)"
participant SETUP as "测试数据准备"
participant DB as "数据库访问(conMysql)"
participant ENC as "数据编码(basicData)"
participant REQ as "请求封装(Request)"
...
### 组件G：重试机制（runFailed）
- 使用`@Retry(max_n=3)`装饰器实现自动重试，减少偶发网络波动影响
- 支持可配置的重试次数和前缀匹配
- 在重试过程中自动调用tearDown和setUp方法

**更新** 新增的重试机制提供了更强大的容错能力。
...
- **更新** 测试生命周期管理：统一的setUp和tearDown方法减少了重复代码，提高了测试执行效率
...
- **更新** 重试失败：检查@Retry装饰器配置，确认重试次数和前缀设置是否正确
...
**更新** 重构后的测试用例具有更好的可维护性和可读性，标准化的测试生命周期管理提高了测试执行的一致性和可靠性。
...
**更新** 测试用例结构标准化要点
  - 使用`@Retry(max_n=3)`装饰器实现自动重试
  - 统一使用`_prepare_test_data`和`_validate_db_state`方法
  - 标准化的测试方法命名（test_01_*, test_02_*, test_03_*）
  - 详细的测试步骤注释和预期结果验证
