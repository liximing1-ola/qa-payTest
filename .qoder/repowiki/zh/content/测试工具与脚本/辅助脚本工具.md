...
- others：外部辅助脚本
  - bot_dinner.py：企业微信机器人点餐提醒
  - bot_tea.py：企业微信机器人茶歇祝福
- common：通用配置与工具
...
- [run_crontab_case.py:1-79](file://run_crontab_case.py#L1-L79)
...
- [Robot.py:1-138](file://Robot.py#L1-L138)
- [run_crontab_case.py:1-79](file://run_crontab_case.py#L1-L79)
- [bot_dinner.py:1-71](file://others/bot_dinner.py#L1-L71)
- [bot_tea.py:1-81](file://others/bot_tea.py#L1-L81)
...
+mode_fail(url, headers, title, reason)
+mode_success(url, headers, reason)
+mode_markdown(url, headers, reason)
+mode_icon(url, headers, reason)
+mode_slack(url, headers, title, reason, color)
+mode_slack_pt(url, headers, title, reason)
}
```
...
### 定时任务与通知集成（run_crontab_case.py）
- 功能概览
  - 自动发现并执行指定目录下的测试用例，支持不同应用（如"APP"、"不夜星球"、"Partying"）的用例集
...
Runner->>Runner : all_case(appInfo)
...
```mermaid
flowchart TD
Start(["执行机器人脚本"]) --> Holiday["判断是否节假日"]
Holiday --> |是| End(["跳过发送"])
Holiday --> |否| BuildMsg["构建消息体"]
BuildMsg --> Send["发送企业微信通知"]
Send --> Mention["@全体成员"]
Mention --> End
```
...
## 结论
本项目提供了完善的辅助脚本工具体系：SLP专用工具与配置为支付测试提供基础能力；Robot统一通知模块实现跨平台播报；定时任务与外部机器人脚本分别覆盖自动化执行与团队提醒场景。通过合理的参数配置与最佳实践，可显著提升测试效率与团队协作效率。
